...
bot.start((ctx) => {
  ctx.reply('Привет! Я помогу рассчитать твою дневную норму калорий и поделюсь полезными рекомендациями. Ты можешь использовать команды в чате, а также открыть приложение по кнопке ниже.', {
    reply_markup: {
      inline_keyboard: [
